...
class MonitoringService : Service() {
    ...
    """.trimIndent()
            ...
            Log.d(TAG, "Sending request to Zabbix API: $jsonBody")  // Log antes de enviar a requisição
            ...
            client.newCall(request).enqueue(object : Callback {
                ...
                override fun onResponse(call: Call, response: Response) {
                    Log.d(TAG, "Received response: ${response.code} ${response.message}")  // Log do código de resposta HTTP
                                ...
                                if (jsonResponse.has("result")) {
                                    ...
                                    if (hostIds.length() > 0) {
                                        ...
                                        if (interfaces.length() > 0) {
                                            interfaceId = interfaces.getJSONObject(0).getString("interfaceid")
                                            lastKnownIp = ip
                                            Log.d(TAG, "Host created successfully with hostId: $hostId and interfaceId: $interfaceId")
                                        } else {
